...
const Header = () => {
  ...
  const timeoutRef = useRef(null); // <— track hover delay timeout
  ...
  const handleDropdownDelayedClose = () => {
    ...
    }, 200); // Delay in ms before hiding submenu
  };
  ...
  return (
    <header className="relative z-50 w-full shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-between px-4 py-3 bg-white border-b sm:flex-nowrap sm:px-6">
        ...
        <div className="flex  justify-end items-center gap-2 sm:ml-[65vw] sm:gap-4">
        ...
        <ul className="flex items-center ml-[55vh] px-8 py-3 gap-6 relative">
          {navItems.map((item, idx) => (
            <li
              ...
              onMouseLeave={() => item.submenu && handleDropdownDelayedClose()}
            >
              {item.submenu ? (
                <>
                  <button
                    ...
                    onBlur={handleDropdownDelayedClose}
                    onClick={() =>
                      setOpenDropdown(
                  ...
                  {openDropdown === item.label && (
                    <ul
                      ...
                      onMouseLeave={handleDropdownDelayedClose}
                    >
                ...
                <a
                  href={item.href}
                  className="transition-colors hover:text-[#9BC211]"
                >
                  {item.label}
                </a>
              )}
      ...
      {mobileMenuOpen && (
        <div
          className="fixed inset-0 z-50 bg-black sm:hidden bg-opacity-40"
            ...
            <ul className="flex flex-col gap-4">
              {navItems.map((item, idx) => (
                <li key={idx} className="relative">
                  {item.submenu ? (
                    <details>
                      <summary className="px-2 py-2 rounded cursor-pointer hover:bg-purple-100">
                        {item.label}
                      </summary>
