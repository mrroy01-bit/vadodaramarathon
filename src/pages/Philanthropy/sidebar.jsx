...
export default function Sidebar({ activeKey, setActiveKey }) {
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          data.forEach((item) => {
            if (
              item.philantropyType &&
              !dynamicCategories.find((cat) => cat.key === item.philantropySlug)
            ) {
              dynamicCategories.push({
                key: item.philantropySlug ||
                     item.philantropyType.toLowerCase().replace(/\s+/g, "-"),
                label: item.philantropyType,
              });
            }
          });
        }
        ...
        console.error("Error fetching philanthropy categories:", error);
  ...
  return (
    <aside className="w-64 h-screen p-4 ml-12 mt-28">
      <h2 className="mb-6 text-xl font-bold text-gray-800">Philanthropy</h2>
