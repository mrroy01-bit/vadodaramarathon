      ...
      try {
        ...
        if (Array.isArray(data)) {
          data.forEach((item) => {
            if (
              item.vmeditionsType &&
              !dynamicCategories.find((cat) => cat.key === item.vmeditionsSlug)
            ) {
              dynamicCategories.push({
                key:
                  item.vmeditionsSlug ||
                  item.vmeditionsType.toLowerCase().replace(/\s+/g, "-"),
                label: item.vmeditionsType,
              });
            }
          });
        }
        ...
        console.error("Error fetching vm editions categories:", error);
